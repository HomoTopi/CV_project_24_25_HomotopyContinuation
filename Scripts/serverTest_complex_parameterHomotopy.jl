...
function process_rectification(params)
    ...
    @var x[1:3], y[1:3], C_1[1:3, 1:3], C_2[1:3, 1:3], C_3[1:3, 1:3]
    ...
    F = System([
            ...
            J_y[2],
            J_y[3]
        ], variables=x ∪ y, parameters=vec(C_1) ∪ vec(C_2) ∪ vec(C_3))

    @info "F = " * string(F)

    #CHeck if the CircularPoints are in the solution space of the base system
    # Evaluate the system Jx and Jy at the base system
    ...
    jx_evaluated = [(
        ...
        J_x,
        vcat(
            [(xi => vi) for (xi, vi) in zip(x, CircularPoints[idx][1:3])],  # Use vi directly
            [(yi => vi) for (yi, vi) in zip(y, CircularPoints[idx][4:6])],  # Use vi directly
            [(C_1i => vi) for (C_1i, vi) in zip(vec(C_1), vec(C_1_base))],  # No change needed
            [(C_2i => vi) for (C_2i, vi) in zip(vec(C_2), vec(C_2_base))],  # No change needed
            [(C_3i => vi) for (C_3i, vi) in zip(vec(C_3), vec(C_3_base))]   # No change needed
        )
        ...
    )) for idx in eachindex(CircularPoints)]
    @info "jx_evaluated=" * string(jx_evaluated)
    ...
    @info "jy_evaluated=" * string(jy_evaluated)

    res = solve(F, [x, y]; show_progress=true, target_parameters=vcat(vec(C_1_base), vec(C_2_base), vec(C_3_base)))
    @info "res_base=" * string(res)

    solutions_base = solutions(res; only_finite=false, only_nonsingular=true)
    @info "solutions_base=" * string(solutions_base)
